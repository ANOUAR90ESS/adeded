          ...
          {articles.map((article) => (
            <div key={article.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden hover:border-purple-500/50 transition-all group flex flex-col h-full hover:shadow-xl hover:shadow-purple-900/10">
              ...
              <div className="p-3 md:p-5 flex flex-col flex-1">
                <h3 
                  className="text-sm md:text-xl font-bold text-white mb-2 md:mb-3 line-clamp-2 group-hover:text-purple-400 transition-colors cursor-pointer"
                  onClick={() => setSelectedArticle(article)}
                >
                  {article.title}
                </h3>
                <p className="text-zinc-400 text-xs md:text-sm mb-3 md:mb-4 line-clamp-3 md:line-clamp-3">
                  {article.description}
                </p>
                   ...
                   <div className="flex justify-between items-center mt-1">
                     ...
                     <div className="flex gap-2 w-full md:w-auto justify-between md:justify-end">
                        <button
                          ...
                          className="p-1.5 md:p-2 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-full transition-colors flex items-center gap-1"
                          title="Share"
                        >
                        ...
                        <button 
                            onClick={() => setSelectedArticle(article)}
                            className="text-purple-400 text-xs md:text-sm font-medium flex items-center gap-1 hover:text-purple-300 transition-colors bg-purple-500/10 hover:bg-purple-500/20 px-2.5 py-1.5 md:px-3 rounded-full"
                        >
